...
class ArtifactService:
    ...
    def publish(self, path: str | Path, *, run_id: str, kind: ArtifactKind = ArtifactKind.OTHER, dataset_id: str | None = None, description: str = "") -> Artifact:
        target = self.workspace.resolve(path, allow_missing=False)
        artifact = Artifact(name=target.name, kind=kind, path=str(target), dataset_id=dataset_id, run_id=run_id, description=description)
