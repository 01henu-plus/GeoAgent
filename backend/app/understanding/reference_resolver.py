...
from app.core.models import Artifact, Dataset, ResolvedReference, Run, StateSnapshot, Task
...
class ReferenceResolver:
    ...
    def resolve(self, message: str, state: StateSnapshot) -> ReferenceResolution:
        references: list[ResolvedReference] = []
        unresolved: list[str] = []
        known = self._known_objects(state)
    ...
    @staticmethod
    def _known_objects(state: StateSnapshot) -> dict[str, object]:
    ...
    @staticmethod
    def _resolve_mention(mention: str, lowered: str, state: StateSnapshot) -> object | None:
        if "任务" in mention:
            if state.active_task_id:
